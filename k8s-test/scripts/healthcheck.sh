...
count=0
while true; do
  NOT_RUNNING=$(kubectl get pods -n $K8_NAMESPACE --field-selector='status.phase!=Running' --no-headers 2>/dev/null | wc -l)
  RUNNING=$(kubectl get pods -n $K8_NAMESPACE --field-selector='status.phase=Running' --no-headers 2>/dev/null | wc -l)
